...
def filasParecidas(matriz: List[List[int]]) -> bool :
  ...
  if len(matriz) > 1:
    elem_parecido: int = matriz[0][0] - matriz[1][0]
    for lista in range(1,len(matriz)):
      for elem in range(0,len(matriz[0])):
        if elem_parecido == matriz[lista-1][elem] - matriz[lista][elem]: 
          pass
        else:
          return False
  else:
